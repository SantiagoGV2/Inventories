...
<div class="container pb-6 pt-6">
	<?php
		...
		$id = (isset($_GET['emple_her_id_up'])) ? $_GET['emple_her_id_up'] : 0;
		$id =limpiar_cadena($id);

		/*== Verificando producto ==*/
    	$check_producto=conexion();
    	$check_producto=$check_producto->query("SELECT * FROM emple_herramientas WHERE emple_her_id='$id'");
	...
	<h2 class="title has-text-centered"><?php echo $datos['emple_her_id']; ?></h2>

	<form action="./php2/her_emple_actualizar.php" method="POST" class="FormularioAjax" autocomplete="off" >

		<input type="hidden" name="emple_her_id" value="<?php echo $datos['emple_her_id']; ?>" required >
		<div class="columns">
				<div class="column">
				    	...
				    	<?php
    						...
    						if($categorias2->rowCount()>0){
    							...
    							foreach($categorias2 as $row){
    								if($datos['her_cod']==$row['her_cod']){
    									echo '<option value="'.$row['her_cod'].'" selected="" >'.$row['her_descripcion'].' (Actual)</option>';
    								}else{
